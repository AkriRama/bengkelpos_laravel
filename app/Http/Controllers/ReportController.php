...
class ReportController extends Controller
{
    public function index(Request $request) {
        ...
        // $report = Transaction::select(DB::raw('DATE(created_at) as date'), DB::raw('sum(total) as sum'))
    ...
    public function storeReport()
    {
        ...
        if(isset($reportSales) && count($reportSales) > 0)
        {
            foreach($reportSales as $report)
            {
                ...
                // // if((isset($reportPurchase) && count($reportPurchase) > 0) || (isset($reportExpenses) && count($reportExpenses) > 0))
                // // {
                ...
                // //     elseif(isset($reportExpenses) && count($reportExpenses) > 0)
                // //     {
        
                // //         foreach($reportExpenses as $reportss)
                // //         {
                // //             if($report->date == $reportss->date)
                // //             {
                // //                 $report = new Report([
                // //                     'date' => $report->date,
                // //                     'sales' => $report->sum,
                ...
                // //                     'income' => $report->sum - $reportss->sum,
                // //                 ]);
                ...
                // elseif((isset($reportPurchase) && count($reportPurchase) > 0) && (isset($reportExpenses) && count($reportExpenses) > 0))
                // {
                //     foreach($reportPurchase as $reports)
                //     {
                //         foreach($reportExpenses as $reportss)
                //         {
                //             if($report->date == $reports->date && $report->date == $reportss->date && $reports->date == $reportss->date)
                //            {
                //                $report = new Report([
                //                    'date' => $report->date,
                //                    'sales' => $report->sum,
                //                    'purchases' => $reports->sum,
                //                    'expenses' => $reportss->sum,
                //                    'income' => $report->sum - $reports->sum - $reportss->sum,
                //                ]);
        ...
        if(isset($reportPurchase) && count($reportPurchase) > 0)
        {
            ...
            foreach($reportPurchase as $reports)
            {
                ...
                if($reportData)
                {
                    $carbonDate = \Carbon\Carbon::parse($reportData->date); // Convert the date string to Carbon
                    $month = $carbonDate->format('m'); // Extract the month from the Carbon date
